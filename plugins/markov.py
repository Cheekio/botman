...
def db_init(db):
    db.execute("create table if not exists markov(prefix, suffix,"
               " primary key(prefix))")
    db.commit()
    db.execute("create table if not exists markov_settings("
...
def get_settings(db):
    settings = db.execute("select * from markov_settings where setting=0"
                          ).fetchone()
    if settings:
        return settings
    else:
...
def set_settings(db, settings):
    db.execute(
        "replace into markov_settings(setting, auto, "
